...
public class SnapshotS3Util extends Configured implements Tool {
    ...
    private String s3aProtocol = S3A_PROTOCOL;
    private String s3nProtocol = S3A_PROTOCOL;
    ...
    private static Options getOptions() {
        ...
        Option useS3a = new Option("a", "s3a", true,
            "Use s3a protocol instead of s3. Might work better");
        Option customS3aEndpoint = new Option("g", "s3.amazonaws.com", true,
            "Use s3a protocol instead of s3. Might work better");
        ...
        useS3a.setRequired(false);
        customS3aEndpoint.setRequired(false);
    ...
    private void printInfo(String effectiveSnapshotName) {
        ...
        LOG.info("s3a protocol    : {}", s3aProtocol);
        LOG.info("s3n protocol    : {}", s3nProtocol);
        LOG.info("s3a endpoint    : {}", s3aEndpoint);
     ...
     *
     * @param snapshotName
     * @return true or false based on the snapshot creation.
     */
    ...
    private boolean importFromS3(String snapshotName) {
        ...
        try {
            ...
            config.set("fs.default.name", s3aProtocol + accessKey + ":" + accessSecret + "@" + bucketName);
            config.set("fs.defaultFS", s3aProtocol + accessKey + ":" + accessSecret + "@" + bucketName);
            config.set("fs.s3a.access.key", accessKey);
            config.set("fs.s3a.secret.key", accessSecret);
            config.set("fs.s3a.endpoint", s3aEndpoint);
        ...
        for (Option option : cmd.getOptions()) {
            switch (option.getId()) {
                ...
                case 'm':
                    ...
                    break;
                case 'a':
                    s3protocol = S3A_PROTOCOL;
                    break;
                case 'g':
                    s3aEndpoint = option.getValue();
                    break;
                case 'l':
